...
public sealed class UiScope protected constructor() {
    ...
    internal companion object {
        /**
         * Creates the private root implementation used by [buildComponentTree].
         *
 ...
 */
@JvmSynthetic
internal fun buildComponentTree(content: UiScope.() -> Unit): Element {
 ...
 *
 * This privileged bridge exists for runtime adapters and structural conformance tests that already own raw [Element] descriptions.
 * Application screens use [dev.s7a.strata.screen.ScreenDefinition], which evaluates its root implicitly.
 * Profile-backed components fail unless an enclosing runtime evaluation is already active on the current thread.
 *
 * @param content callback that must emit exactly one root.
...
public fun evaluateComponentTree(content: UiScope.() -> Unit): Element = buildComponentTree(content)
