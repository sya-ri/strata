...
val compatibilityMarkdown = providers.provider {
    buildString {
        appendLine("<!-- Generated by :generateCompatibilityDocumentation. Do not edit. -->")
        appendLine()
        ...
        appendLine("Use the aligned Strata release version shown in the [installation example](../../README.md#installation).")
        appendLine("For changes between Strata releases, read the [changelog](../../CHANGELOG.md).")
        appendLine()
...
val generateCompatibilityDocumentation = tasks.register("generateCompatibilityDocumentation") {
    group = "documentation"
    description = "Generates supported Minecraft artifacts and Java requirements from the typed target matrix."
    dependsOn(verifyMinecraftFabricTargetMatrix)
    inputs.property("markdown", compatibilityMarkdown)
    outputs.file(compatibilityDocumentation)
    doLast {
        compatibilityDocumentation.asFile.parentFile.mkdirs()
        compatibilityDocumentation.asFile.writeText(compatibilityMarkdown.get())
    }
}
val checkCompatibilityDocumentation = tasks.register("checkCompatibilityDocumentation") {
    group = "verification"
    description = "Checks the generated compatibility reference without changing tracked documentation."
    dependsOn(verifyMinecraftFabricTargetMatrix)
    inputs.property("markdown", compatibilityMarkdown)
    inputs.file(compatibilityDocumentation)
    doLast {
        check(compatibilityDocumentation.asFile.readText() == compatibilityMarkdown.get()) {
            "The compatibility reference is stale. Run generateCompatibilityDocumentation."
        }
    }
}
...
val sharedSourceMarkdown = providers.provider {
    val sourcesByVersion = minecraftFabricTargets.associate { target ->
        val runtime = project(target.runtimeProjectPath)
        val sources = runtime.extensions.getByType<SourceSetContainer>().named("main").get().allSource
        target.version to sources.srcDirs
    ...
    val rootsByRole = sourcesByVersion.values.flatten().distinct().sorted()
    ...
    buildString {
        ...
        appendLine()
        appendLine("Rows are Minecraft versions; columns are shared source roots compiled into each runtime's main source set.")
        appendLine("A check marks a selected root. These are source directories, not separately installed or published libraries.")
        appendLine("Tables are grouped by responsibility; paths are relative to `runtime/shared/minecraft-fabric/`.")
...
val generateDokkaModuleMarkdown =
    tasks.register("generateDokkaModuleMarkdown") {
        ...
        val template = layout.projectDirectory.file("docs/publication/dokka-module.md")
...
extensions.configure<StrataReleaseExtension> {
    ...
    modrinthProjectBodyFile.set(layout.projectDirectory.file("docs/publication/modrinth-project.md"))
