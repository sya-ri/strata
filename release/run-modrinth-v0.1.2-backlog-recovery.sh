...
[[ -n "$operation" && -n "$contract_file" && -n "$project_id" && -n "$release_tag" ]] || \
  fail 'Usage: run-modrinth-v0.1.2-backlog-recovery.sh <operation> <contract> <project-id> <tag> <source-commit> <controller-commit> <pages-record>'
...
[[ "$runner_path" == "$controller_tool_directory/run-modrinth-v0.1.2-backlog-recovery.sh" && \
  "$contract_resolved" == "$controller_tool_directory/modrinth-v0.1.2-backlog-recovery.json" ]] || \
...
expected_source_commit='b541fc5492b798b6805c0c4d24e09f43ceff938a'
expected_source_object='49195293b3e163abd0beefc9fc8e61a428b8eb24'
expected_source_body_blob='075d941895ed9b70bcb42d9814948fb1bdc4a05d'
backlog_body_sha256='d036e6399abd278d3fb0dfb2740749f2458302df3c84f503f0b3bf9051350a33'
predecessor_body_sha256='3e79db82a612df73b387d1bc5a4b8b1f495ff0ae9042d1a9bae7a01da14bda1b'
current_body_sha256='f214e060534509ae59024a4bd9295d39a07c29e7f1665d2a9e943a90fe7f34f9'
...
jq -e \
  ...
  --arg current "$current_body_sha256" '
    ...
    .releaseSource == {
      tag: "v0.1.2",
      commit: "b541fc5492b798b6805c0c4d24e09f43ceff938a",
      tagObject: "49195293b3e163abd0beefc9fc8e61a428b8eb24",
      bodyBlob: "075d941895ed9b70bcb42d9814948fb1bdc4a05d",
      releaseVersion: "0.1.2",
      artifactCount: 21,
      manifestCanonicalSha256: "5a26f471445f2e53d5c24e6431bf902d5d659e449a9d295d3c199fa132a757c6"
    } and
    .bodyLineage == {
    ...
    .baselineReleases[1].bodySha256 == $predecessor and
  ...
  ' "$contract_file" >/dev/null || fail 'The Modrinth backlog recovery contract differs from the reviewed v0.1.2 contract.'
...
[[ "$project_id" == 'CAdZ3jVr' && "$release_tag" == 'v0.1.2' && "$source_commit" == "$expected_source_commit" ]] || \
  fail 'The requested Modrinth backlog release identity differs.'

[[ "$(git rev-parse HEAD)" == "$expected_source_commit" ]] || fail 'The checked-out product source is not exact v0.1.2.'
[[ "$(git rev-parse "refs/tags/v0.1.2^{commit}")" == "$expected_source_commit" ]] || fail 'The v0.1.2 tag commit differs.'
[[ "$(git rev-parse refs/tags/v0.1.2)" == "$expected_source_object" ]] || fail 'The v0.1.2 tag object differs.'
[[ "$(git rev-parse "$expected_source_commit:docs/modrinth-project.md")" == "$expected_source_body_blob" ]] || \
  fail 'The v0.1.2 Modrinth body blob differs.'
...
temporary="$(mktemp -d "$runner_parent/strata-modrinth-v012-backlog.XXXXXX")"
...
cleanup() {
  ...
  trap - EXIT INT TERM
  set +e
  for historical_worktree in "${historical_worktrees[@]}"; do
    ...
    git worktree remove --force "$historical_worktree" >/dev/null 2>&1 || cleanup_failed=1
  done
  if [[ "$manifest_was_patched" == '1' ]]; then
    cp "$manifest_backup" "$manifest" || cleanup_failed=1
    if [[ -n "$original_manifest_sha256" && "$(sha256sum "$manifest" | cut -d ' ' -f 1)" != "$original_manifest_sha256" ]]; then
...
bash release/verify-release-tag.sh v0.1.2 "$expected_source_commit" "$expected_source_object" >/dev/null
...
[[ -f "$manifest" && ! -L "$manifest" ]] || fail 'The canonical v0.1.2 Modrinth manifest is missing.'
...
[[ "$canonical_manifest_sha256" == "$expected_manifest_sha256" ]] || \
  fail 'The canonical v0.1.2 Modrinth manifest differs from its pinned canonical hash.'
...
jq -e \
  ...
  ' "$manifest" >/dev/null || fail 'The canonical v0.1.2 Modrinth manifest identity differs.'
...
[[ "$(sha256sum "$manifest_body" | cut -d ' ' -f 1)" == "$current_body_sha256" ]] || \
  fail 'The canonical v0.1.2 Modrinth body differs.'
...
jq -e 'length == 21 and ([.[].version_number] | length == (unique | length))' "$current_expected_core" >/dev/null || \
  fail 'The canonical v0.1.2 Modrinth core contract inventory differs.'
...
[[ "$(wc -l < "$baseline_versions" | tr -d '[:space:]')" == '41' ]] || fail 'The pinned backlog baseline must contain 41 versions.'
[[ "$(wc -l < "$current_versions" | tr -d '[:space:]')" == '21' ]] || fail 'The v0.1.2 manifest must contain 21 versions.'
[[ "$(wc -l < "$allowed_versions" | tr -d '[:space:]')" == '62' ]] || fail 'The backlog union must contain 62 unique versions.'

fetch_state() {
  local prefix="$1"
  local require_current="$2"
  local lifecycle_policy="$3"
  ...
  jq -e --arg project "$project_id" --arg lifecycle "$lifecycle_policy" '
    .id == $project and
    .slug == "strata-ui" and
    .project_type == "mod" and
    (if $lifecycle == "processing" then
      .status == "processing" and .requested_status == "approved"
    else
      (.status == "processing" and .requested_status == "approved") or .status == "approved"
    end)
  ...
  [[ -z "$(comm -23 "$baseline_versions" "$remote_versions")" ]] || fail 'The remote Modrinth backlog is missing a v0.1.0 or v0.1.1 version.'
  ...
  if [[ "$require_current" == '1' ]]; then
    [[ -z "$(comm -23 "$current_versions" "$remote_versions")" ]] || fail 'The remote Modrinth backlog is missing a v0.1.2 version.'
  fi
  jq '
      def normalize_changelog:
        gsub("\r\n"; "\n") | sub("[[:space:]]+$"; "") + "\n";
  ...
  jq -e 'length == 41' "$baseline_metadata" >/dev/null || fail 'The historical Modrinth metadata projection must contain 41 versions.'
  ...
  if [[ -s "$baseline_expected_core" ]]; then
    ...
    [[ "$baseline_core_sha256" == "$(jq -cS . "$baseline_expected_core" | tr -d '\r' | sha256sum | cut -d ' ' -f 1)" ]] || \
      fail 'Historical Modrinth version metadata differs from the two signed release manifests.'
  fi
  jq --slurpfile versions "$current_versions_json" '
      [.[] | select(.version_number as $number | ($versions[0] | index($number)) != null)] | sort_by(.version_number)
  ...
  [[ "$current_core_sha256" == "$(jq -cS . "$current_expected_subset" | tr -d '\r' | sha256sum | cut -d ' ' -f 1)" ]] || \
    fail 'Existing Modrinth v0.1.2 metadata differs from the exact signed release manifest.'
...
IFS=$'\t' read -r status_before requested_status_before inventory_before count_before baseline_metadata_before current_metadata_before baseline_core_before current_core_before < \
  <(fetch_state before "$require_current_before" processing)

verify_historical_remote 0 20
verify_historical_remote 1 21
jq --slurp 'add | sort_by(.version_number)' \
  "$temporary/historical-0-core.json" "$temporary/historical-1-core.json" > "$baseline_expected_core"
jq -e 'length == 41 and ([.[].version_number] | length == (unique | length))' "$baseline_expected_core" >/dev/null || \
  fail 'The combined signed historical Modrinth core contract must contain 41 unique versions.'
...
IFS=$'\t' read -r status_boundary requested_status_boundary inventory_boundary count_boundary baseline_metadata_boundary current_metadata_boundary baseline_core_boundary current_core_boundary < \
  <(fetch_state boundary "$require_current_boundary" processing)
[[ "$status_boundary" == "$status_before" && "$requested_status_boundary" == "$requested_status_before" && \
...
case "$operation" in
  ...
  stage)
    ...
    IFS=$'\t' read -r status_write requested_status_write inventory_write count_write baseline_metadata_write current_metadata_write baseline_core_write current_core_write < \
      <(fetch_state write-boundary 0 processing)
    [[ "$status_write" == "$status_boundary" && "$requested_status_write" == "$requested_status_boundary" && \
...
IFS=$'\t' read -r status_after requested_status_after inventory_after count_after baseline_metadata_after current_metadata_after baseline_core_after current_core_after < \
  <(fetch_state after "$require_current_after" transition)
if [[ "$require_current_after" == '1' ]]; then
  [[ "$current_core_after" == "$current_core_contract_sha256" ]] || \
    fail 'The complete Modrinth v0.1.2 inventory differs from the signed release core contract.'
fi
...
if [[ "$operation" == 'stage' ]]; then
  [[ "$count_after" == '62' ]] || fail 'The staged Modrinth backlog must contain exactly 62 release versions.'
  ...
  [[ "$(jq -cS . "$after_existing_current_metadata" | tr -d '\r' | sha256sum | cut -d ' ' -f 1)" == "$current_metadata_write" ]] || \
    fail 'The append-only Modrinth stage changed a pre-existing v0.1.2 version identity or file URL.'
fi
...
write_scope='none'
if [[ "$operation" == 'stage' ]]; then
  write_scope='missing-v0.1.2-versions-only'
fi
...
jq --null-input \
  ...
  '{
    ...
    historicalPreflights: [$historical0[0], $historical1[0]],
...
echo "Modrinth v0.1.2 backlog $operation completed without changing the shared project body or tagged artifacts."
