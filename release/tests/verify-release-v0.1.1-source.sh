...
grep --fixed-strings 'release/controller-overlays/v0.1.0-central-signature-checksums.json' <<< "$predecessor_block" >/dev/null || fail 'The audited v0.1.0 Central manifest is not loaded from the exact controller.'
grep --fixed-strings 'release/controller-overlays/v0.1.0-central-signature-checksums.patch' <<< "$predecessor_block" >/dev/null || fail 'The audited v0.1.0 Central patch is not loaded from the exact controller.'
