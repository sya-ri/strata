...
[[ "$(wc -l < "$inventory")" == "41" ]] || fail 'The GitHub Release fixture does not contain 41 assets.'
jq --raw-input --slurp 'split("\n") | map(select(length != 0)) | to_entries | map({name: .value, id: (.key + 1)})' \
  "$inventory" > "$assets_json"
[[ "$(jq 'length' "$assets_json")" == "41" ]] || fail 'The GitHub Release fixture asset metadata is incomplete.'
...
write_release_json() {
  ...
  [[ "$(jq '.assets | length' "$release_json")" == "41" ]] || fail 'The GitHub Release response fixture is incomplete.'
}
