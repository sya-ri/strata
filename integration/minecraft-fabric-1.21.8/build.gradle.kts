...
evaluationDependsOn(":runtime:minecraft-fabric-1.21.8")
val runtimeFabricProject = project(":runtime:minecraft-fabric-1.21.8")
...
fabricApi {
    configureTests {
        ...
        modId = "strata-integration-minecraft-fabric-1-21-8"
...
tasks.named<ProcessResources>("processGametestResources") {
    ...
    inputs.property("minecraftVersion", libs.versions.minecraft1218)
    inputs.property("integrationModId", "strata-integration-minecraft-fabric-1-21-8")
    inputs.property("runtimeModId", "strata-runtime-minecraft-fabric-1-21-8")
    filesMatching("fabric.mod.json") {
        expand(
            "version" to project.version,
            "minecraftVersion" to libs.versions.minecraft1218.get(),
            "integrationModId" to "strata-integration-minecraft-fabric-1-21-8",
            "runtimeModId" to "strata-runtime-minecraft-fabric-1-21-8",
        )
    }
}

dependencies {
    minecraft(libs.minecraft1218)
    mappings(loom.officialMojangMappings())
    modImplementation(libs.fabric.loader)
    modImplementation(libs.fabric.api1218)
    ...
    add("productionRuntimeMods", libs.fabric.api1218)
...
val runProductionClientGameTest = tasks.register<ClientProductionRunTask>("runProductionClientGameTest") {
    ...
    dependsOn(deleteProductionGameTestRunDir, ":runtime:minecraft-fabric-1.21.8:remapJar")
    ...
    jvmArgs.add(libs.versions.minecraft1218.map { version -> "-Dstrata.minecraftVersion=$version" })
}
...
tasks.named<JavaExec>("runClientGameTest") {
    ...
    doFirst {
        ...
        systemProperty("strata.minecraftVersion", libs.versions.minecraft1218.get())
    }
}
