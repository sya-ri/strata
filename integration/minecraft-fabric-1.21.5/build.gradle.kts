...
evaluationDependsOn(":runtime:minecraft-fabric-1.21.5")
val runtimeFabricProject = project(":runtime:minecraft-fabric-1.21.5")
...
fabricApi {
    configureTests {
        ...
        modId = "strata-integration-minecraft-fabric-1-21-5"
...
tasks.named<ProcessResources>("processGametestResources") {
    ...
    inputs.property("minecraftVersion", libs.versions.minecraft1215)
    inputs.property("integrationModId", "strata-integration-minecraft-fabric-1-21-5")
    inputs.property("runtimeModId", "strata-runtime-minecraft-fabric-1-21-5")
    filesMatching("fabric.mod.json") {
        expand(
            "version" to project.version,
            "minecraftVersion" to libs.versions.minecraft1215.get(),
            "integrationModId" to "strata-integration-minecraft-fabric-1-21-5",
            "runtimeModId" to "strata-runtime-minecraft-fabric-1-21-5",
        )
    }
}

dependencies {
    minecraft(libs.minecraft1215)
    mappings(loom.officialMojangMappings())
    modImplementation(libs.fabric.loader)
    modImplementation(libs.fabric.api1215)
    ...
    add("productionRuntimeMods", libs.fabric.api1215)
...
val runProductionClientGameTest = tasks.register<ClientProductionRunTask>("runProductionClientGameTest") {
    ...
    dependsOn(deleteProductionGameTestRunDir, ":runtime:minecraft-fabric-1.21.5:remapJar")
    ...
    jvmArgs.add(libs.versions.minecraft1215.map { version -> "-Dstrata.minecraftVersion=$version" })
}
...
tasks.named<JavaExec>("runClientGameTest") {
    ...
    doFirst {
        ...
        systemProperty("strata.minecraftVersion", libs.versions.minecraft1215.get())
    }
}
