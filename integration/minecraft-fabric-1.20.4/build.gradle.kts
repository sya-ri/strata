...
evaluationDependsOn(":runtime:minecraft-fabric-1.20.4")
val runtimeFabricProject = project(":runtime:minecraft-fabric-1.20.4")
...
fabricApi {
    configureTests {
        ...
        modId = "strata-integration-minecraft-fabric-1-20-4"
...
tasks.named<ProcessResources>("processGametestResources") {
    ...
    inputs.property("minecraftVersion", libs.versions.minecraft1204)
    inputs.property("integrationModId", "strata-integration-minecraft-fabric-1-20-4")
    inputs.property("runtimeModId", "strata-runtime-minecraft-fabric-1-20-4")
    filesMatching("fabric.mod.json") {
        expand(
            "version" to project.version,
            "minecraftVersion" to libs.versions.minecraft1204.get(),
            "integrationModId" to "strata-integration-minecraft-fabric-1-20-4",
            "runtimeModId" to "strata-runtime-minecraft-fabric-1-20-4",
        )
    }
}

dependencies {
    minecraft(libs.minecraft1204)
    mappings(loom.officialMojangMappings())
    modImplementation(libs.fabric.loader)
    modImplementation(libs.fabric.api1204)
    ...
    add("productionRuntimeMods", libs.fabric.api1204)
...
val runProductionClientGameTest = tasks.register<ClientProductionRunTask>("runProductionClientGameTest") {
    ...
    dependsOn(deleteProductionGameTestRunDir, ":runtime:minecraft-fabric-1.20.4:remapJar")
    ...
    jvmArgs.add(libs.versions.minecraft1204.map { version -> "-Dstrata.minecraftVersion=$version" })
}
...
tasks.named<JavaExec>("runClientGameTest") {
    ...
    doFirst {
        ...
        systemProperty("strata.minecraftVersion", libs.versions.minecraft1204.get())
    }
}
