...
/**
 * Restores native surface notifications and scoped focus handling in the verified Fabric client-test input harness.
 *
...
public final class MinecraftCanvasWindowTestPlugin implements IMixinConfigPlugin {
    ...
    public void postApply(String targetClassName, ClassNode targetClass, String mixinClassName, IMixinInfo mixinInfo) {
        ...
        restoreSurfaceNotification(targetClass);
    ...
    /**
     * Restores the native surface notification omitted by Fabric's synthetic resize path.
     * The verified upstream call already runs after physical SDL resize with updated framebuffer dimensions.
     * Native framebufferSizeChanged invalidates RenderPearl's surface before performing the same resizeGui call.
     */
    private static void restoreSurfaceNotification(ClassNode targetClass) {
        List<MethodInsnNode> calls = new ArrayList<>();
        for (MethodNode method : targetClass.methods) {
            AnnotationNode merged = Annotations.getVisible(method, MixinMerged.class);
            if (merged == null || FABRIC_WINDOW_MIXIN.equals(Annotations.<String>getValue(merged, "mixin")) == false
                || "fabric_resize".equals(method.name) == false || "(II)V".equals(method.desc) == false) {
                continue;
            }
            for (AbstractInsnNode instruction : method.instructions) {
                if (instruction instanceof MethodInsnNode call
                    && Type.getInternalName(WindowEventHandler.class).equals(call.owner)
                    && "resizeGui".equals(call.name) && "()V".equals(call.desc)) {
                    calls.add(call);
                }
            }
        }
        if (calls.size() != 1) {
            throw new IllegalStateException("Expected Fabric's single native GUI resize notification.");
        }
        calls.get(0).name = "framebufferSizeChanged";
    }
    ...
    private static String findNativeFocus(ClassNode targetClass) {
        List<MethodInsnNode> calls = new ArrayList<>();
        for (MethodNode method : targetClass.methods) {
            if ("strataCanvasFocus".equals(method.name) && "(Z)V".equals(method.desc)) {
                for (AbstractInsnNode instruction : method.instructions) {
                    if (instruction instanceof MethodInsnNode call && targetClass.name.equals(call.owner) && "(Z)V".equals(call.desc)) {
                        calls.add(call);
                    }
                }
            }
