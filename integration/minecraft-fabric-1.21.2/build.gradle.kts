...
evaluationDependsOn(":runtime:minecraft-fabric-1.21.2")
val runtimeFabricProject = project(":runtime:minecraft-fabric-1.21.2")
...
fabricApi {
    configureTests {
        ...
        modId = "strata-integration-minecraft-fabric-1-21-2"
...
tasks.named<ProcessResources>("processGametestResources") {
    ...
    inputs.property("minecraftVersion", libs.versions.minecraft1212)
    inputs.property("integrationModId", "strata-integration-minecraft-fabric-1-21-2")
    inputs.property("runtimeModId", "strata-runtime-minecraft-fabric-1-21-2")
    filesMatching("fabric.mod.json") {
        expand(
            "version" to project.version,
            "minecraftVersion" to libs.versions.minecraft1212.get(),
            "integrationModId" to "strata-integration-minecraft-fabric-1-21-2",
            "runtimeModId" to "strata-runtime-minecraft-fabric-1-21-2",
        )
    }
}

dependencies {
    minecraft(libs.minecraft1212)
    mappings(loom.officialMojangMappings())
    modImplementation(libs.fabric.loader)
    modImplementation(libs.fabric.api1212)
...
val runProductionClientGameTest = tasks.register<ClientProductionRunTask>("runProductionClientGameTest") {
    ...
    dependsOn(deleteProductionGameTestRunDir, ":runtime:minecraft-fabric-1.21.2:remapJar")
    ...
    jvmArgs.add(libs.versions.minecraft1212.map { version -> "-Dstrata.minecraftVersion=$version" })
}
...
tasks.named<JavaExec>("runClientGameTest") {
    ...
    doFirst {
        ...
        systemProperty("strata.minecraftVersion", libs.versions.minecraft1212.get())
    }
}
