...
/**
 * Shows 5 players before a scroll viewport is introduced, deliberately exposing the list overflow.
 * The unopened definition reads caller-owned immutable inputs on its host thread and propagates layout failures.
 */
internal fun fivePlayersScreen(
    players: List<ReadmePlayer>,
    panel: ImageSource,
): ScreenDefinition =
    ReadmeDemoChrome.screen(panel, playerCount = 5) { panelModifier, rowModifier ->
        // readme-demo:start
