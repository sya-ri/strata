...
internal enum class ReadmeDemoStage(
     ...
     */
    Basic("Start with a row", "Each row takes the width of its contents.", "BasicPlayersExample", 300, ::basicPlayersScreen),

    /**
     * Secondary text expands natural width and recenters the name with its role.
     */
    Roles("Add secondary text", "One Text adds a role to every player.", "RolesPlayersExample", 400, ::rolesPlayersScreen),
     ...
     */
    Right("Align text to the right", "End aligns names and roles; row frames, faces, and buttons stay still.", "RightPlayersExample", 400, ::rightPlayersScreen),

    /**
     * Restore text alignment without moving the row, face, or button.
     */
    Left("Align text to the left", "Start moves only the text back to the left.", "LeftPlayersExample", 200, ::leftPlayersScreen),

    /**
     * The first extra player exposes the need for a bounded scroll viewport.
     */
    Four("Add a fourth player", "The same row composition is reused as the list starts to overflow.", "FourPlayersExample", 40, ::fourPlayersScreen, 4),

    /**
     * Fixed offline data grows by one row per short frame.
     */
    Five("Add a fifth player", "Players arrive one at a time.", "FivePlayersExample", 40, ::fivePlayersScreen, 5),

    /**
     * The unscrollable list continues growing.
     */
    Six("Add a sixth player", "The list is now taller than the screen.", "SixPlayersExample", 40, ::sixPlayersScreen, 6),

    /**
     * Overflow remains intentional until the scroll-area chapter.
     */
    Seven("Add a seventh player", "Additional rows cannot be reached yet.", "SevenPlayersExample", 40, ::sevenPlayersScreen, 7),

    /**
     * The complete roster precedes either scrolling component.
     */
    Eight("Add an eighth player", "Eight players now need a scroll viewport.", "EightPlayersExample", 40, ::eightPlayersScreen, 8),

    /**
     * A scroll area clips and scrolls the existing column without requiring a scrollbar.
     */
    Area("Add a scroll area", "Wrap the existing Column in ScrollArea to contain the list.", "AreaPlayersExample", 200, ::areaPlayersScreen, 8),

    /**
     * Real wheel input reaches the last player before any scrollbar exists.
     */
    AreaBottom("Scroll without a bar", "Wheel input reaches the final players; no scrollbar has been added yet.", "AreaPlayersExample", 100, ::areaPlayersScreen, 8, listOf(208)),

    /**
     * The independent scrollbar observes the same state at the current list offset.
     */
    Bar("Add a linked scrollbar", "Pass the same ScrollState to Scrollbar; its thumb reflects the list position.", "ScrollPlayersExample", 200, ::scrollPlayersScreen, 8, listOf(208)),

    /**
     * Both list pixels and scrollbar thumb follow upward wheel input.
     */
    BarTop("Scroll up together", "The list and linked scrollbar move together under wheel input.", "ScrollPlayersExample", 150, ::scrollPlayersScreen, 8, listOf(168, 126, 84, 42, 0)),

    /**
     * Return to the last player, then hold the completed source and screen for two seconds.
     */
    BarBottom("Scroll down together", "The linked thumb follows the list back to its last player.", "ScrollPlayersExample", 350, ::scrollPlayersScreen, 8, listOf(42, 84, 126, 168, 208, 208)),
}
