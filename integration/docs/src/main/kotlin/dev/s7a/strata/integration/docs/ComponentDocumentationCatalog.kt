...
internal object ComponentDocumentationCatalog {
    ...
    internal fun summary(component: DocumentedComponent): String =
        when (component) {
            ...
            DocumentedComponent.Button -> "Button renders verified fixed-height Minecraft sprite, label, and enabled semantic states, including the native 150- and 200-pixel widths. It owns no implicit focus or activation, while reusable input actions live in modifiers."
    ...
    internal fun parentScopeGuidance(component: DocumentedComponent): String =
        when (component) {
            ...
            DocumentedComponent.Button -> "`Button` is a top-level extension on the active `UiScope`. The screen runtime installs its selected Minecraft profile only for the definition callback, while caller-owned activation and pointer modifiers remain valid only through their retained modifier-node lifetime."
