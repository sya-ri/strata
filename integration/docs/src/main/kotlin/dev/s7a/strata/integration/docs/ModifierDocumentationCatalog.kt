...
internal object ModifierDocumentationCatalog {
    ...
    internal val entries: Map<String, Entry> =
        mapOf(
            ...
            "onActivate" to Entry("Activation", "Runs one shared action for a primary pointer press or each focused Enter or Space press, makes the owner focusable, and adds no node or action reference when its enabled overload is false."),
            ...
            "onPress" to Entry("Pointer", "Handles a pointer press or invokes a simple action overload; use `onActivate` instead when primary pointer and focused keyboard input represent the same action."),
