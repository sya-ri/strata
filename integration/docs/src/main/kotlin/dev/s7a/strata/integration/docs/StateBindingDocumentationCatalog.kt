...
internal object StateBindingDocumentationCatalog {
    ...
    internal val entries: List<Entry> =
        listOf(
            ...
            Entry("PanZoomFit", "PanZoomFit.kt", "Contain or cover policy defining the base scale represented by zoom one."),
            Entry("PanZoomMetrics", "PanZoomMetrics.kt", "Immutable current center, zoom, resolved scale, viewport, content bounds, and fit snapshot."),
            Entry("PanZoomState", "PanZoomState.kt", "Caller-owned owner-thread transform with bounded pan, anchored zoom, coordinate conversion, and one live viewport geometry owner."),
            Entry("TiledImageCachePolicy", "TiledImageCachePolicy.kt", "Positive per-attachment tile-entry and byte limits plus bounded overscan."),
            Entry("TiledImageLevel", "TiledImageLevel.kt", "Immutable tile pixel extent and content-unit scale for one source resolution level."),
            Entry("TiledImageSource", "TiledImageSource.kt", "Externally owned source generation with immutable bounds, levels, and independently revisioned tile histories."),
            Entry("TiledImageTile", "TiledImageTile.kt", "Empty or immutable ready presentation state for one tile revision."),
            Entry("TiledImageTileId", "TiledImageTileId.kt", "Signed row and column identity within one source-owned resolution level."),
