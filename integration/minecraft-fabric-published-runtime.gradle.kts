    ...
    doFirst {
        ...
        ZipFile(artifact.file).use { archive ->
            ...
            check(nestedJars == expectedNestedJars) {
                "Published runtime must contain exactly the canonical common jars: $nestedJars"
            }
            ...
            check(declaredNestedJars == expectedNestedJars.map { name -> "META-INF/jars/$name" }.sorted()) {
                "Published runtime metadata must reference exactly the canonical common jars: $declaredNestedJars"
            }
        }
    }
    doLast {
