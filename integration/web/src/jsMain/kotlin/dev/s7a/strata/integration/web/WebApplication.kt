...
public fun main() {
    if (WebLaunchMode.decode(window.location.search) == WebLaunchMode.Prerender) {
        val html = renderWebDocument(ReactiveScenario().definition(), ReactiveScenario.viewport, "Strata runtime parity", "application.js")
        ...
        val host = mountWeb(ReactiveScenario().definition(), root, ReactiveScenario.viewport)
