...
/**
 * Runs a real unobfuscated screen extraction against an independently owned GUI state.
 *
...
internal fun extractMinecraftCanvasScreen(
    ...
    val graphics = GuiGraphicsExtractor(Minecraft.getInstance(), state, 0, 0)
    before()
    return attemptMinecraftCanvasExtraction({ screen.extractRenderState(graphics, 0, 0, 0f) }) {
        var elements = 0
