...
- `runtime:minecraft-fabric-26.2` is the client-only boundary for the current latest Java release.
  ...
  Its loaded client GameTest compares native screens using the actual menu background, generic container, Slot highlights, font, EditBox, Button, and `ObjectSelectionList` assets and widgets against both the Fabric adapter and the common headless compositor with exact ARGB equality, then compares the custom industrial Mod screen through the same Fabric/headless pixels.
- `integration:minecraft-fabric-26.2` owns the loaded client vanilla parity scenes, synchronized inventory Slot scenario, custom resource-pack-backed industrial screen, compiled Minecraft-component examples, and build-only verification evidence; it is not published.
...
It covers `ConfirmScreen`, `DirectJoinServerScreen`, `ContainerScreen`, an actual `ObjectSelectionList`, and a custom industrial Mod screen, then writes the verified full frames and typed Slot, Text, TextField, Button, Scroll, and Image crops with their hashes below its build directory.
