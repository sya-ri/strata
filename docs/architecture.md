...
- `runtime:minecraft-fabric-1.21.11` through `runtime:minecraft-fabric-1.21` are client-only Java 21 boundaries for the older remapped distributions.
  All twelve compile against official Mojang mappings and reuse the resource, profile, font, text, lifecycle, native-neutral input, inventory, and current-screen behavior proven by every supported legacy compiler and loaded client.
  ...
  The 1.21.1 and 1.21 projects isolate ABGR native-image access and the earlier texture blit signature, while the profile boundary selects typed legacy Slot, Tooltip, and horizontal-progress treatments from actual release capabilities rather than version strings.
...
- `runtime:minecraft-fabric-1.21.3-legacy` isolates the direct player-skin future used by 1.21.3 through 1.21 before the optional result boundary and owns the compatible primitive key-binding bridge without source filtering.
...
- `integration:minecraft-fabric-1.21.11` through `integration:minecraft-fabric-1.21` compile the complete runner-independent `integration/minecraft-fabric-1.21-legacy` suite against their exact remapped Java 21 boundaries; none is published.
  ...
  The `integration/minecraft-fabric-client-gametest` source root adapts Fabric Client GameTest for 1.21.4 and later, while `integration/minecraft-fabric-1.21.3-legacy` supplies an ordinary client entrypoint, tick synchronization, disposable world ownership, and failure propagation for 1.21.3 through 1.21 before that Fabric module existed.
