...
- `runtime:minecraft` is a publishable Minecraft-independent adapter boundary built on `runtime:core`.
  ...
  Button and Tab own profile-backed appearance and enabled semantics without implicit focus or activation, while platform-neutral active modifiers own raw pointer, keyboard, committed-character, preedit, focus, logical activation, press, release, move, drag, scroll, and hover actions reusable by future Minecraft components.
  ...
  `onActivate(enabled)` composes primary-pointer and focused Enter-or-Space input into one action and adds no node when disabled; raw `onPress` remains available for pointer-specific behavior, and hover changes only in response to delivered pointer movement.
...
The generator validates the complete declared manifest and asset index for each load, then records the immutable client, selection contract, logical resource path sets, consumed resource hashes, source hashes, logical viewports, GUI scales, physical PNG dimensions, and image origins in `docs/components/headless-render.properties` before staging Markdown and PNG output.
Aggregate manifest and asset-index hashes are validation inputs rather than receipt identities, so content revisions to unrelated existing mappings that preserve the logical path set do not make unchanged showcase output stale.
