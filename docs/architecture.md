...
- `runtime:minecraft-fabric-1.21.11` through `runtime:minecraft-fabric-1.20.5` are client-only Java 21 boundaries for the older remapped distributions, and `runtime:minecraft-fabric-1.20.4` is the corresponding Java 17 boundary.
  All fifteen compile against official Mojang mappings and reuse the resource, profile, font, text, lifecycle, native-neutral input, inventory, and current-screen behavior proven by every supported legacy compiler and loaded client.
  ...
  The 1.20.4 project additionally owns its `GameProfile`-based skin resolution and key-binding bridges, selects its legacy active-pack menu/list/separator assets by capability, and reproduces the black scrollbar track drawn directly by that client.
...
- `integration:minecraft-fabric-1.21.11` through `integration:minecraft-fabric-1.20.4` compile the complete runner-independent `integration/minecraft-fabric-1.21-legacy` suite against their exact remapped Java 21 or Java 17 boundaries; none is published.
