...
- `runtime:minecraft-fabric-1.21.11` through `runtime:minecraft-fabric-1.20.5` are client-only Java 21 boundaries for the older remapped distributions.
  All fourteen compile against official Mojang mappings and reuse the resource, profile, font, text, lifecycle, native-neutral input, inventory, and current-screen behavior proven by every supported legacy compiler and loaded client.
...
- `integration:minecraft-fabric-1.21.11` through `integration:minecraft-fabric-1.20.5` compile the complete runner-independent `integration/minecraft-fabric-1.21-legacy` suite against their exact remapped Java 21 boundaries; none is published.
