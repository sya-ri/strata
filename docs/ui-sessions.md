...
TextField owns the verified EditBox sprites, typed profile-backed text colors, native insert or append cursor, bounded printable-ASCII editing policy, and semantics while caller-owned owner-thread `MinecraftTextFieldState` owns the value and maximum length.
...
The Fabric-backed `Slot(bind = ...)` form accepts `MinecraftSlots.playerInventory(index)`, a logical `MinecraftSlots.container(index)`, or the raw-menu escape hatch `MinecraftSlots.activeMenu(index)`; it polls the current authoritative menu before each frame, inserts native item rendering at the Slot's ordered item phase, and sends pointer transactions through Minecraft's container-input operation instead of mutating inventory storage.
