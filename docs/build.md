...
The `runtime:minecraft-fabric-1.20.4` module applies the same publication and packaging contract with the Java 17 toolchain required by that release.
All seventeen Fabric runtime modules declare an exact Minecraft requirement and catalog-derived Loader, Fabric Language Kotlin, and Java lower bounds; none uses Fabric API at runtime.
...
The 1.20.4 project keeps the compatible complete roots while owning its pre-`ResolvableProfile` skin and key-binding bridges; its profile selects the active legacy GUI resources and reproduces the code-defined scrollbar track without version-string dispatch.
...
Configuration on demand is enabled so a targeted API, core, headless, documentation-helper, or benchmark task does not configure all 34 Loom projects.
