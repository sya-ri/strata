...
`publication-status.yml` observes successful publication receipts on completion and hourly, then requests one protected verification run when all selected distributions become public.
It uses anonymous Modrinth/Hangar reads and a repository-scoped CurseForge read key; distribution upload tokens stay in the protected release job.
