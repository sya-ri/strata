...
- the common Minecraft runtime owns one-shot screen definitions, immutable asset profiles, callback-scoped menu/container/image background modifiers and Slot/Text/TextField/Button/Scroll/Image/PlayerHead components, including a Fabric-backed Slot overload synchronized with the active player's inventory, and a synchronous fixed-viewport host over the retained core;
- the latest Java release, Minecraft 26.2, has a Fabric boundary that extracts the supported native profile, resolves Mod images and current-player skin pixels through the active resource and texture paths, and adapts common frames, typed mouse/keyboard/text input, and screen lifecycle on the client thread; loaded client GameTests verify exact native/Fabric/headless ARGB parity for vanilla screens and PlayerHead plus exact Fabric/headless parity for a resource-pack-backed industrial Mod screen.
...
- `integration/minecraft-fabric-26.2` runs loaded client parity scenes against actual 26.2 resources, vanilla screens, the selected player skin, the synchronized inventory Slot, and a custom resource-pack-backed industrial screen; it is not published.
