...
- the common Minecraft runtime owns one-shot screen definitions, immutable asset profiles, callback-scoped menu/container/image background modifiers and Slot/Text/TextField/Button/Scroll/Image components, including a Fabric-backed Slot overload synchronized with the active player's inventory, and a synchronous fixed-viewport host over the retained core;
- the latest Java release, Minecraft 26.2, has a Fabric boundary that extracts the supported native profile, resolves Mod images through the active resource-pack stack, and adapts common frames, typed mouse/keyboard/text input, and screen lifecycle on the client thread; loaded client GameTests verify exact native/Fabric/headless ARGB parity for vanilla screens and exact Fabric/headless parity for a resource-pack-backed industrial Mod screen.
...
- `runtime/minecraft` is configured as a publishable Minecraft-independent one-shot screen definition, immutable profile, callback-scoped menu/container/image background modifiers, Slot/Text/TextField/Button/Scroll/Image component set, shared structural asset identifiers, declarative bindings for live player-inventory and active-menu Slots, an opaque version-platform bridge, and an owner-thread host over the retained core.
...
- `integration/minecraft-fabric-26.2` runs loaded client parity scenes against actual 26.2 resources, vanilla screens, the synchronized inventory Slot, and a custom resource-pack-backed industrial screen; it is not published.
