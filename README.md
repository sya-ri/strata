...
- the latest Java release, Minecraft 26.2, has a Fabric boundary that extracts the supported native asset/font profile and adapts common frames, input, and screen lifecycle on the client thread; a loaded client GameTest verifies exact native/Fabric/headless ARGB parity.
...
- `integration/minecraft-fabric-26.2` runs the loaded client parity scene against actual 26.2 resources and native widgets; it is not published.
