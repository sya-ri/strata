 ...
 */
internal fun paperScreenService(
    plugin: Plugin,
    dispatchEvent: (Event) -> Unit = { plugin.server.pluginManager.callEvent(it) },
    ...
    RemoteScreenService(
        ...
        notify = { player, event -> dispatchEvent(paperUiEvent(player, event)) },
    )
