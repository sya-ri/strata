...
internal class RemoteConnectionTest {
    ...
    fun terminalSessionReleasesQueuedAndPartiallyDeliveredMessagesWithoutBreakingTheConnection() {
        ...
        sender.send(RemoteMessage.Action(1, 1, 1, BuiltinProjection.PointerPress.type, ProjectionValue.Text("x".repeat(4000))))
        sender.flush(1)
        assertNull(receiver.receive(frames.removeFirst(), 1))
        sender.send(RemoteMessage.Action(1, 2, 1, BuiltinProjection.PointerPress.type, ProjectionValue.Absent))
