...
internal class ScreenDefinitionTest {
    @Test
    fun constructionIsLazyAndCloseDoesNotEvaluateContent() {
        var contentCalls = 0
        val definition =
            ScreenDefinition(UiText.Literal("lazy")) {
                contentCalls += 1
    ...
    fun transferExposesMetadataAndEvaluatesContentOnlyOnAttach() {
        ...
        var contentCalls = 0
        val definition =
            ScreenDefinition(title, pausesGame = true) {
                contentCalls += 1
    ...
    fun definitionCanBeTransferredExactlyOnce() {
        val definition =
            ScreenDefinition(UiText.Literal("one")) {
                Text("A")
            }
        val profile = MinecraftProfileFixture.create()
        val host = createMinecraftUiHost(definition, profile)
    ...
    fun transferAndCloseHaveExactlyOneWinner() {
        ...
        repeat(20) {
            val definition =
                ScreenDefinition(UiText.Literal("race")) {
                    Text("A")
                }
            val profile = MinecraftProfileFixture.create()
    ...
    fun closingAnAvailableDefinitionDropsItsPayload() {
        val definition =
            ScreenDefinition(UiText.Literal("closed")) {
                element(MinecraftHostProbe().element())
            }
        definition.close()
    ...
    private fun transferredStateClass(): Class<*> {
        val definition =
            ScreenDefinition(UiText.Literal("transferred-state")) {
    ...
    private fun closedStateClass(): Class<*> {
        val definition =
            ScreenDefinition(UiText.Literal("closed-state")) {
                MinecraftHostProbe().element()
            }
        definition.close()
    ...
    private fun assertPayloadReleased(definition: ScreenDefinition) {
        ...
        assertTrue(state.javaClass.declaredFields.none { field -> field.type == ScreenDefinitionPayload::class.java })
    }
