...
val previousRuntime = rootProject.file("runtime/minecraft-fabric-1.20.4/src/main")

extensions.configure<SourceSetContainer> {
    named("main") {
        java.srcDirs(previousRuntime.resolve("java"), sharedRuntime.resolve("java"), legacyRuntime.resolve("java"))
...
extensions.configure<KotlinJvmProjectExtension> {
    sourceSets.named("main") {
        kotlin.srcDirs(
            previousRuntime.resolve("kotlin"),
            sharedRuntime.resolve("kotlin"),
            legacyRuntime.resolve("kotlin"),
            primitiveInputRuntime.resolve("kotlin"),
        )
    }
}
...
tasks.named<ProcessResources>("processResources") {
    ...
    inputs.property("javaVersion", libs.versions.java.baseline)
    filesMatching("fabric.mod.json") {
        expand(
            "version" to project.version,
            "fabricLoader" to libs.versions.fabric.loader.get(),
            "minecraft" to libs.versions.minecraft1203.get(),
            "fabricLanguageKotlin" to libs.versions.fabric.language.kotlin.get(),
            "java" to libs.versions.java.baseline.get(),
        )
    }
}
...
val verifyFabricModArtifact = tasks.register("verifyFabricModArtifact") {
    ...
    doLast {
        ...
        ZipFile(artifact).use { archive ->
            ...
            check(metadata["id"] == "strata-runtime-minecraft-fabric-1-20-3") {
...
loom {
    mods {
        register("strata-runtime-minecraft-fabric-1.20.3") {
