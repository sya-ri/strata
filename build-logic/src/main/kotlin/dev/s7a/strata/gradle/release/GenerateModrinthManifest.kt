...
@CacheableTask
internal abstract class GenerateModrinthManifest @Inject constructor(
    ...
    fun generate() {
        ...
        val changelog = releaseNotesFile.get().asFile.readText().replace("\r\n", "\n").trimEnd() + "\n"
        check(changelog.isNotBlank()) { "Release notes must not be blank." }
        val projectBody = projectBodyFile.get().asFile.readText().replace("\r\n", "\n").trimEnd() + "\n"
